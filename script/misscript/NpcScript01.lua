...
function for_newbie()
	...
	Talk( 3, "Чтобы получить Медаль новичка, которая дает +5% к максимальным жизням, принеси мне:_1.Карта мини-босса Москвы - 10;_2.Золото - 100.000. " )
	InitTrigger()
	TriggerCondition( 1, HasItem,8125,10 ) --Это функция проверки какой итем и в каком количестве должен быть в рюкзаке
	TriggerCondition( 1, HasMoney, 100000 )
	TriggerAction( 1, TakeItem, 8125,10 ) -- Это функция "изьятия" предмета и в каком количестве
	TriggerAction( 1, TakeMoney, 100000 )
	TriggerAction( 1, GiveItem, 8154,1,4 ) -- Это функция выдачи предмета (ID, кол-во)
	TriggerFailure( 1, JumpPage, 6 )
	Text( 3, "Обменять Медаль новичка. ", MultiTrigger, GetMultiTrigger(), 1)
	Text( 3, "Уйти. ", CloseTalk )
