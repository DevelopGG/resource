...
----------------
	...
	local Check_A = math.max ( 0.02 , math.min ( 1 , ( 1.1 - Baoshi_NeedLv * 0.1 + Sklv * 0.15 +ZBforgeXG ) ) )
		...
		if Baoshi_NeedLv == 3 or Baoshi_NeedLv == 4 or Baoshi_NeedLv == 5 or Baoshi_NeedLv == 6 or Baoshi_NeedLv ==7 or Baoshi_NeedLv == 8 or Baoshi_NeedLv== 9 then  ---------????????
	...
	if CheckFaild == 1 then
		...
		R1 = RemoveChaItem ( role , ItemID_Stone1 , 1 , 2 , ItemBag [1] , 2 , 1 , 0 )		--????1
		R2 = RemoveChaItem ( role , ItemID_Stone2 , 1 , 2 , ItemBag [2] , 2 , 1 , 0 )		--????2
...
--	Notice(Num)
	...
	--if ResetCheck == 0 then
		---Notice("ResetCheck??")
		--LG("check_item_final","ResetCheck??")
		--return
	--end 
	StoneInfo[0]=0
	StoneInfo[1]=0
...
--	SystemNotice( arg[1] , "????????")
	...
	local Item_Cailiao1 = GetChaItem ( role , 2 , ItemBag [1] )					--??????1???
	local Item_Cailiao2 = GetChaItem ( role , 2 , ItemBag [2] )					--??????2???
	...
	local MJdamo_ItemID = GetItemID (Item_damo)--------???????????
	...
	local BB95_qiecaidaoId = GetItemID (Item_damo)--------????????????
	...
	local BB95_sheriId = GetItemID (Item_damo)--------?????????????
	...
	local BB95_ak47Id = GetItemID (Item_damo)--------?????????AK47
	...
	local BB95_shoushudaoId = GetItemID (Item_damo)--------????????????
	...
	local BB95_dakandaoId = GetItemID (Item_damo)--------????????????
	...
	local BB95_laonainaiId = GetItemID (Item_damo)--------???????????????
	...
	local BB95_yadiannaId = GetItemID (Item_damo)--------??????????????
	...
	local MianJu = GetItemID (Item_damo)--------???????????
...
--	Notice ("???"..Money_Need.."?????")
	...
	R1 = RemoveChaItem ( role , ItemID_Cailiao1 , 1 , 2 , ItemBag [1] , 2 , 1 , 0 )		--????1
	R2 = RemoveChaItem ( role , ItemID_Cailiao2 , 1 , 2 , ItemBag [2] , 2 , 1 , 0 )		--????2
...
function can_beuplv_item_main ( Table )
	...
	local Item_beuplv_ID = GetItemID ( Item_beuplv )		----?????ID
	local Item_ScItem = GetChaItem ( role , 2 , ItemBag [0] )	----????????
	local Item_YxItem = GetChaItem ( role , 2 , ItemBag [2] )	----????????
	local Item_ScItem_ID = GetItemID ( Item_ScItem )		----????ID
	local Item_YxItem_ID = GetItemID ( Item_YxItem )		----????ID
	...
	local ItemAttr_Val_Fusionid = GetItemAttr ( Item_beuplv , ITEMATTR_VAL_FUSIONID ) -- ?????????ID
	----SystemNotice(role,"????="..Item_beuplv_Lv)

	if Item_beuplv_ID < 5001 or Item_beuplv_ID > 8000 then					--?????????????
...
function Check_CG_beuplv ( Item_Lv )				-------?????????
	...
	if Item_Lv <= 10 then					-------10???100%??
		return 1
	end
	if Item_Lv>10 and Item_Lv <= 15 then			-------10?15???50%??
		if ran <= 50 then
		return 1
		else
		return 0
	...
	if Item_Lv>15 and Item_Lv <= 20 then			-------15?20???10%??
		if ran <= 10 then
		return 1
		else
		return 0
...
function can_jlborn_item_main ( Table )
	...
	local Item_JLone_ID = GetItemID ( Item_JLone )   --??????ID
	local Item_JLother_ID = GetItemID ( Item_JLother )   --??????ID
	---??????????
...
function jlborn_item ( Table )
	...
	local Item_JLone = GetChaItem ( role , 2 , ItemBag [1] )					--?????????
	local Item_JLother = GetChaItem ( role , 2 , ItemBag [2] )				--??????????
	local  Item_EMstone_ID = GetItemID ( Item_EMstone )					 --????ID
	local  Item_JLone_ID = GetItemID ( Item_JLone )						 --??????ID
	local  Item_JLother_ID = GetItemID ( Item_JLother )					 --??????ID
	---??????????
	local str_JLone = GetItemAttr( Item_JLone ,ITEMATTR_VAL_STR )			  --?? 
...
function GetChaName1_guildwar( role,npc )-------------????????
	...
	--AddState ( role , role , STATE_JRQKL , 10 , 180 )
        AddState ( role , role , STATE_QINGZ , 10 , 300 )---5??15%????
	elseif a ==2 then
	--AddState ( role , role , STATE_JRQKL , 10 , 180 )
	AddState ( role , role , STATE_YS , 10 , 300 )---5????
	elseif a ==3 then
	--AddState ( role , role , STATE_JRQKL , 10 , 180 )
	AddState ( role , role , STATE_HFZQ , 10 , 10 )---10??hp?sp??
	else
	AddState ( role , role , STATE_JRQKL , 10 , 180 )---3?????????
	end
    end
    if num_count < 20 then
...
function GetChaName5_guildwar(role)-------??15????
...
function GetChaName6_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 15 )
...
function GetChaName7_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 15 )
...
function GetChaName8_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 15 )
...
function GetChaName9_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 15 )
...
function GetChaName10_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 15 )
...
--function GetChaName11_guildwar(role)-------??15????(??)
--     local num_count_1 = CheckBagItem(role, 176)----??????
--     local map_name_cha = GetChaMapName ( role )
--     if num_count_1 >= 15 and map_name_cha == "guildwar" then
...
function GetChaName11_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
...
function GetChaName12_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 30 )
...
function GetChaName13_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 30 )
...
function GetChaName14_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 30 )
...
function GetChaName15_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 30 )
...
function GetChaName16_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 30 )
...
function GetChaName17_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
...
function GetChaName18_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 45 )
...
function GetChaName19_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 45 )
...
function GetChaName20_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 45 )
...
function GetChaName21_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 45 )
...
function GetChaName22_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 2964)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,2964, 45 )
...
function GetChaName27_guildwar(role)-------??15????
...
function GetChaName28_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 15 )
...
function GetChaName29_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 15 )
...
function GetChaName30_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 15 )
...
function GetChaName31_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 15 )
...
function GetChaName32_guildwar(role)-------??15????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 15 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 15 )
...
function GetChaName33_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
...
function GetChaName34_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 30 )
...
function GetChaName35_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 30 )
...
function GetChaName36_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 30 )
...
function GetChaName37_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 30 )
...
function GetChaName38_guildwar(role)-------??30????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 30 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 30 )
...
function GetChaName39_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
...
function GetChaName40_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 45 )
...
function GetChaName41_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 45 )
...
function GetChaName42_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 45 )
...
function GetChaName43_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 45 )
...
function GetChaName44_guildwar(role)-------??45????
     local num_count_1 = CheckBagItem(role, 3001)
     local map_name_cha = GetChaMapName ( role )
     if num_count_1 >= 45 and map_name_cha == "guildwar" then
        TakeItem( role, 0,3001, 45 )
...
function GetChaName_born ( role )
	...
	-----SystemNotice(role ,"?????=="..cha_skill_num)
...
------?2????????????	
	...
	if ItemType_JLSX ~= 22 and ItemType_JLSX ~=27 then ----??????????
	star = 1
	end
...
--------???????????????
	...
	if ItemAttr_Val_Fusionid1 ~= 0 and ItemAttr_Val_Fusionid2 ~= 0 then					--?????????????
...
----------------==============
	local Jinglianxinxi = GetItemForgeParam ( Item_JLSX , 1 )
		...
		R1 = RemoveChaItem ( role , ItemID_Shizhongyan , Shizhongyan_ItemCount , 2 , ItemBag [1] , 2 , 1 , 0 )		--??????
		R2 = RemoveChaItem ( role , ItemID_JLSX , 1 , 2 , ItemBag [0] , 2 , 1 , 0 )		--??????
...
--	Notice("????")
	...
	j = RemoveChaItem ( role , ItemID2 , 1 , 2 , BagItem2 , 2 , 1 , 0)		--???????
...
function TransferDiamondScript_Lv1 ( role )
	...
	local x_give = 0 
	local y_give = 0 
	local script_count = CheckBagItem ( cha , 888 )					--??????????
	local DiamondId = Roll_DiamondId ( cha )						--?????????
	if script_count >= 1 then 
...
function MegaBuff( role )
	...
	if money == 0 then
		PopupNotice(role, "×òîáû ïîëó÷èòü ïîëîæèòåëüíûå ýôôåêòû, äàé ìíå 150 òûñÿ÷ çîëîòûõ. ")
		return 0
	end
	...
	if (StateType1 == 0 and  StateType2 == 0 and  StateType3 == 0 and  StateType4 == 0 and  StateType5 == 0 and  StateType6 == 0) then
		...
		PopupNotice( role ,"Âû ïîëó÷èëè çàðÿä ïîëîæèòåëüíîé ýíåðãèè íà 1 ÷àñ! ") 
	else

		PopupNotice(role,"Áàôôû óæå íàëîæåíû! ") 
