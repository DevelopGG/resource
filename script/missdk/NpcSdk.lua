...
dofile(GetResPath("beach/Functions.lua"))
dofile(GetResPath("ptb/Functions.lua"))
print( "------------------------------------" )
...
function MsgProc( character, npc, rpk, page, trade, missionlist, exchangeData )
	...
	elseif usCmd == CMD_CM_FUNCITEM then --功能页选项命令信息处理
		...
		----------------------------------------------------
		-- // Custom Function: Plant the Bomb Register // --
		elseif item.func == PTB.Register then
			return PTB.Register(character)
		----------------------------------------------------	
		elseif item.func == Beach.Register then
			return Beach.Register(character)
		...
		-- Колизей --
		-------------
		elseif item.func == Coliseum.Register then
			return Coliseum.Register(character,npc)
		------------------
		-- Призы за топ --
		------------------
