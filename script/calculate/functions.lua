...
function DefSb(a)
	...
	--LG("setchaattr","???",ATTR_STATEV_DEF,"??? = ", defsb,"\n" ) 
	return defsb 
end 
...
function Check_Direction(a) --????????,x??????1????,??????2,3,4,5,6,7,8
	--LuaPrint("Enter function Check_Direction(a) --????????,x??????1????,??????2,3,4,5,6,7,8") 
	...
	--LuaPrint("Out function Check_Direction(a) --[[????????,x??????1????,??????2,3,4,5,6,7,8?]--") 
	return division 
end 
...
function Hp_Dmg(role,dmg) --[[??hp??????]]--
	...
	--LuaPrint("Enter function Hp_Dmg(role,dmg) --[[hp??????]]--") 
	--Notice("dmg3="..dmg)
	...
	SetCharaAttr(hp, role, ATTR_HP )  
	--LuaPrint("Out function Hp_Dmg(role,dmg) --[[hp??????]]--") 
end 
...
function Sp_Red(role,sp_reduce) --[[sp??????]]--
	--LuaPrint("Enter function Sp_Red(role,spreduce) --[[sp??????]]--") 
	local sp = Sp(role) - sp_reduce 
	--LG("sp_red", "before_sp = " , Sp(role) , " sp_reduce = " , sp_reduce , "final_sp = " , sp ,"\n" ) 
	SetCharaAttr(sp, role, ATTR_SP ) 
	--LuaPrint("Out function Sp_Red(role,spreduce) --[[sp??????]]--") 
end 
...
function SetSteady_atk(a) --[[????????]]--
	--LuaPrint("Enter function SetSteady_atk(a) --[[????????]]--") 
	...
	--gatk = gatk + i * ccc --[[?????]]--
	--end 
	--LuaPrint("Out function SetSteady_atk(a) --[[????????]]--") 
	return atk,sum 
end 
	
	
function CheckSteady_atk(a) --[[??????]]--
	--LuaPrint("Enter function CheckSteady_atk(a) --[[??????]]--") 
	...
	--LuaPrint ("??!???????,???????".."\n") 
	--LuaPrint("Out function CheckSteady_atk(a) --[[??????]]--") 
...
function MGPK_Dead_Punish_Exp ( dead )
	local dead_lv=GetChaAttr(dead, ATTR_LV)							--100???????????	
	if dead_lv>=100 then
...
--	SystemNotice ( Cha_role , "?????"..attr_num )
--	SystemNotice ( Cha_role , "???"..Num )
...
--	SystemNotice ( Cha_role , "????"..attr_num_1 )
	if i == 0 then
...
--	SystemNotice ( role_want_repair , "??"..Money_Need )
...
function get_item_repair_money ( role_want_repair,Item )
	...
	local ZBday = GetChaAttr(role_want_repair,ATTR_EXTEND2)		--????
...
function GiveGoldenMapItem_JLB ( role )
	...
	local monserID=math.random(a,b)				--------??????829?836
...
function cha_timer(role, freq, time) 
	...
	if math.mod(now_tick, resume_freq) == 0 and now_tick > 0 then
		...
		CHTLD(role, now_tick)--???
		MSG_TEST("2")
		MoJianBag(role,now_tick)-------?????
		MoJianHand(role,now_tick)--------?????
		MSG_TEST("3")
	...
	if math.mod(now_tick, 30) == 0 and now_tick > 0 then  --2008??????????   ???????30?1?
		WLface(role,now_tick)             --2008??????????   ????? ?buff5?1?	
	end
	...
	if Item_SBNum == 1 then
		...
		if SB_Type == 1 and SB_Open == 0 then
			...
			if Now_Time >= SB_Time then
				...
				--SystemNotice(role,"??,Now_Time==="..Now_Time)	
...
-- function XieXieHa(role,now_tick)
	-- local crab = GetChaItem ( role , 2 , 1  ) --???????????
	-- local crab_ID= GetItemID (crab)				--???????ID		
	-- if crab_ID==58 then				--????ID?58 ??
		-- local ENERGY = GetItemAttr(crab, ITEMATTR_ENERGY)			--?????????
		-- local ENERGY_resume = 1			--?????????1
		-- if ENERGY <= 0 then
			-- ENERGY = 0
		-- end
				-- if ENERGY ==0 then			--?????0
...
------------------------------------kokora------------------------------------------
	...
	--if math.mod(now_tick, Rwine_freq) == 0 and now_tick > 0 then 
		--local Rwine = GetChaItem ( role , 2 , 1  ) --???????????
		--local Rwine_ID= GetItemID (Rwine)				--???????ID
		
		--if Rwine_ID==2967 then				--????ID?2967 ??
			...
			--if ENERGY ==0 then			--?????0
	 		...
	 		--else 					--??????0
				--ENERGY = ENERGY - ENERGY_resume  		--?????????
				--SetItemAttr(Rwine, ITEMATTR_ENERGY, ENERGY )			--???????????????????  URE
			--end		
		--end
--end

---------------------------------------------------------------------------------??200
	--[[local one_minite=60														--??one_minite?60?
		...
		for i=5786,5790,1 do 
			...
			if num_moonbox_feijiu==1 then
				...
				local item_moonbox_ure=GetItemAttr(item_moonbox,ITEMATTR_URE)
						--SystemNotice ( role , "?????????????"..item_moonbox_ure )			
		...
		if map_name == "prisonisland" then
			...
			if item_id == 5724 then			----??????????
				...
				elseif energy ==0 then			--?????0
					...
					energy = energy - energy_resume  		--?????????
					SetItemAttr(item_jiluka, ITEMATTR_ENERGY, energy )			--???????????????????  URE
				end
			end
		end
	end	
...
function JLAutoSpeed(role,elf)
	local autoSpeed = GetChaItem ( role , 2 , 2  ) --???????????
...
function GiveJLCoins(role,elf,num1)
	...
	if lv >=JLMINLV and lv <JLMAXLV then 
		...
		if math.mod( num , 1) == 0 and ure>=50 and is2==1 then
			GiveItemX ( role , 0 , 855 , num1 , 4 )										-------??1???????
		end
		if math.mod( num , 2) == 0 and ure>=50 and is2~=1 then
			GiveItemX ( role , 0 , 855 , num1 , 4 )										-------??2???????
		end
		-- if math.mod( num , 30) == 0  and ure>=50 and is2==1 then			-------??30?????????
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 60) == 0  and ure>=50 and is2~=1 then			-------??60?????????
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 120) == 0  and ure>=50  then						-------??120?????????
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 1200) == 0  and ure>=50 then						-------??1200?????????
	...
	-- if CheckDateNum > 8121416 then
		...
		-- if bb85_6473_InhandId == 6473 then
			...
			-- if bb85_6473_JinglianLevel < 15 then
				...
				-- LG("lianzhonghuodong",ChaName,"??ID=6473,????=",bb85_6473_JinglianLevel,"\n")
				-- return
			-- end
		-- end
	-- end
...
function MoJianBag(role,now_tick)--------------?????
	...
	-- SystemNotice(role,"InnnnnnBAOBAO2222222??"..mojiantime)
...
function KouMoHeTime(role,now_tick)
	if math.mod(now_tick, 30) == 0 and now_tick > 0 then	
		...
		if mohetime < 241 then
			...
			if now_day~=IDay then--??????MAXENERGY??1
				-- SystemNotice(role,"mohetime======="..mohetime)
...
function Give_ElfEXP_MISSION ( role , num )						--??????????
	...
	if ure <= 50 then									
		...
		SystemNotice (role , CALCULATE_FUNCTIONS_LUA_000132)			--????50???????exp
		return 0 
	else 
		ure = ure - 40								--?????,????40???,????EXP
		SetItemAttr ( Item , ITEMATTR_URE , ure ) 
...
-- function  TigerStart( ... )
	...
	-- if reset_rad>=47 and num[1]==num[3] and num[1]==num[5] and num[1]==num[7]  and num[1]==num[9]  then-------??????39%
		-- num[9]=kapian_rad
	-- end
	-- if reset_rad>=24 and num[2]==num[4] and num[2]==num[5] and num[2]==num[6]  and num[2]==num[8]  then-------??????66%
		-- num[8]=kapian_rad
	-- end
	-- if  num[1]==num[2] and num[1]==num[3] and num[1]==num[4]  and num[1]==num[6] and num[1]==num[7] and num[1]==num[8]  and num[1]==num[9] then-------??????66%
...
--		Notice ( "??"..ChaName.."????????,???????"..Item_Name)
--		LG ( "JLYBBOX" , "??"..ChaName.."??????????"..Item_Name)
	if a >= 1 and a <= 10000000 then						------?0.1%??
		XSAddExpAll ( role , 1, 1, 2 )
	elseif a >= 10000001 and a <= 12200000 then					------?0.2%??
...
function AddExpPer ( role , value,value2 )			----------??????????????????
	if value2 == nil then
		value2 =  value
	--	SystemNotice (role , "??: value2===="..value2)
	end
...
function Checkjlktime ( role )
	...
	if jiluka_num == 1 then
		local jiluka = GetChaItem2 (  role , 2 , 5724 )			--?????????ID?5724??????
...
function Winterland_SetMisTime ( role )			----------??????????????????
	local now_month= os.date("%m")		-------------?
	local now_day= os.date("%d")		-------------?
	now_month= tonumber(now_month)		-------------?     
	now_day= tonumber(now_day)		-------------?     
...
function Reg_Player_Event_Name ( Event_Name )			--?????  ??1 ??  0??
...
function Add_Player_Event_Value( Event_Name , Role , Do_Day ,Has_Record )			--?????????????
    if Event_Name == "Test" then
		return 0
	end
	local i = 0
...
function Get_Index_By_Event_And_PlayerName(Event_Name , Role)				--????????????????????,???????????  ??0??????????? ??????????index ?
	   local Role_Name = GetChaDefaultName(Role)
	   local Array_Length = table.getn(Player_Can_Do)
	   for i =1 , Array_Length , 1 do
	   		if Player_Can_Do[i].Event_Name == Event_Name then
			...
			elseif  Player_Can_Do[i].Event_Name ~= Event_Name and i==Array_Length then		--?????
				Reg_Player_Event_Name(Event_Name)			--??????
				return 0
			end 
	   end
end

function Clear_Event_Record_By_Event_Name( Event_Name )									--???????????????false
	local Array_Length = table.getn(Player_Can_Do)
	local Now_Time = Day_Mission_Time()
	for i =1 , Array_Length , 1 do
   		if Player_Can_Do[i].Event_Name == Event_Name then
...
function JL_lv (role)-------------------------?????????1?
	...
	if  Get_Item_Type ~= 59 then
		...
		local agi_Traget = GetItemAttr( Item_bg ,ITEMATTR_VAL_AGI )		--??
...
function  Get_ItemAttr_Join1(role)----????????????????1
	local RYZ_Num = 0
	RYZ_Num =  CheckBagItem( role,3849 )
	if RYZ_Num ~= 1 then
		--SystemNotice(role ,"????????????????")
...
function  Get_ItemAttr_Join20(role)----????????????????5
	local RYZ_Num = 0
	RYZ_Num =  CheckBagItem( role,3849 )
	if RYZ_Num ~= 1 then
		return LUA_FALSE
...
function JLTLZM (role)    ---------??????????
	...
	if  Get_Item_Type ~= 59 then
		...
		local Item_Traget_MAXURE=GetItemAttr(Item_bg,ITEMATTR_MAXURE)   --?????
...
-----------------------------------------------??????????
function GiveItem_SetAttr (character,p1,p2,p3,p4,p5,p6)		--p1:??ID,p2=??,p3=BUFF??,p4=BUFF????,p5=???,p6=BUFF??
...
function ShuangBei_TimeChaxun (role)       -----????????
...
------------------------------------------------------------??????
function ShuangBei_Time (role , p1 ,p2 )			--p1 ??
...
function ShuangBei_ChunChu (role)
	...
	SetItemAttr(Have_pingzi, ITEMATTR_VAL_STR, p  )	--????
...
function ZhanBu (role,p1)    -------p1 1?????? 2?????? 3??RMB??
	if p1==2 then 
...
ZBStateBGL[1][10] = 1000	ZBStateBGL[2][10] = 1000	ZBStateBGL[3][10] = 1000	ZBStateBGL[4][10] = 1000	ZBStateBGL[5][10] = 1000	ZBStateBGL[6][10] = 1000		
	...
	if p1>=4 and p1<=6 then   --?4?6? ???5%?????EXP???????????
...
---------------------------------------------------?????????
function CheckItemAttr( role , p1, p2)----item_csk:????
	local ItemAttr={}
	ItemAttr[1]=ITEMATTR_VAL_STR    ----  ????
	ItemAttr[2]=ITEMATTR_VAL_AGI    ----  ????
	ItemAttr[3]=ITEMATTR_VAL_DEX    ----  ????
	...
	if item_num ~= 1 then
		...
		item = GetChaItem2 (  role , 2 , p1 )-----------------?????
		local itemattr = GetItemAttr(item, ItemAttr[3])--------????p2??
...
function CheckIsMapFull(character)
	...
	local MapName = {}
		...
		if Mapstate[i] == 0 then
			...
			--SystemNotice(character,"CaptainConfirm????")	
		...
		elseif 	Mapstate[i] == 1 then
			...
			--SystemNotice(character,"csk_teamid - ??="..csk_teamid)
			--CaptainConfirm(character, csk_teamid )
			--CaptainConfirmAsk(p1, csk_teamid)
			--CaptainConfirmAsk(character, csk_teamid)
			--SystemNotice(character,"CaptainConfirm????")	
...
--[   ???   ????  ]--   by - dean
function ArenaTeamSignUP(character)         --??
	local retbag = HasLeaveBagGrid( character, 1)         --?????1??,??????
	if retbag ~= LUA_TRUE then
		...
		--SystemNotice (character,"????????")
	local teamid =  GetAmphitheaterNoUseTeamID()           --????????????
	...
	local roundid =  GetAmphitheaterRound()
		...
		--SystemNotice (character,"????????")    
	local player1 = GetTeamCha( character , 0 )				--?????2??????
		--SystemNotice (character,"??????1???")
	local player2 = GetTeamCha( character , 1 )				--?????3??????
		--SystemNotice (character,"??????2???")
	AmphitheaterTeamSignUP(teamid,character,player1,player2)	--????????????????
		--SystemNotice(character,"????????")
	local r1=0
	local r2=0
		r1,r2 =MakeItem ( character , 6066  , 1 , 4 )		--??????????????
		--SystemNotice(character,"NPC??????")
	local csk_num = GetChaItem ( character , 2 , r2 )
		SetItemAttr(csk_num , ITEMATTR_VAL_STR , teamid)	--????????????
		SetItemAttr(csk_num , ITEMATTR_VAL_AGI , seasonid)	--??????????
		--SystemNotice(character,"??season????")
		SetItemAttr(csk_num , ITEMATTR_VAL_DEX , roundid)	--??????????
		--SystemNotice(character,"??round????")
		SynChaKitbag(character , 41)                             --????????
		return LUA_TRUE			
end

------------?????[[ ???????????????????? ]]  by - dean
function CheckIsValidTeam(character) --     -1 ????, -2 ????3??, -3  ????
	...
	if Sing_Time ~= 1 then
		...
		local ValidTeam = IsValidTeam(character)                 ---????????,??,???????????
			--SystemNotice(character , "???="..ValidTeam)
		local team_num_now = GetAmphitheaterTeamCount()          ---??? [[  ????????? ]] 
			--SystemNotice( character , "????="..team_num_now)
		local csk_num = CheckBagItem( character, 6066 )	
		if csk_num ~= 0 then
...
function ZBNSDstate(role,now_tick)-------------???
	local now_day=os.date("%d")
	now_day=tonumber(now_day)
	local ZBMain= GetChaAttr(role,ATTR_EXTEND0)
	...
	if math.mod(now_tick, 30) == 0 and now_tick>0 then
		...
		if IsDeb==0 and ZBNMain==1 then
			...
			-- if RDa<=ZBStateDEBGL[8] and RDa > ZBStateDEBGL[7] then 
				...
				-- local b=1-a	
				-- SystemNotice(role,"????"..sex[a].."??????????????????"..sex[b].."“?????”?,??????,?????????????20%%??")
				-- return
			-- end					
		end
	end
...
function XSAddExpAll ( role, value1, value2, type)	------------------????(???)valu1=1,value2=1??0.1%
--	SystemNotice( role , "value1="..value1.."value2="..value2.."type="..type)
	local exp_now = GetChaAttr ( role, ATTR_CEXP )
	if type == 1 then							--------??????
		local exp_add = math.random ( value1, value2)
...
function UseAexpItem(role,Item,statelv,statetime)
	...
	AddState(role,role,STATE_SBJYGZ,statelv,statetime)
	--SystemNotice(role,"??:???????"..statelv.."???????"..statetime)
	return 1
end
function CheckCanUseSB(role,Item,statelv,statetime)
	...
	local ItemID = GetItemID (Item)
	local itemName = GetItemName (ItemID)
	--SystemNotice(role,"??:???????"..SBstate)
	--SystemNotice(role,"??:????????"..SBStateYXJ[SBstate])
	--SystemNotice(role,"??:????????"..SBStateYXJ[statelv])
...
function AddexpItem(role,Item,statelv,statetime,NTC) --ntC: NTC?1???????NOTICE,?0??????????????BUFF???????,??0???BUFF,??1 ??BUFF
...
function CheckCanAddBL(role,Item,statelv,statetime,NTC)
	...
	local ItemID = GetItemID (Item)
	local itemName = GetItemName (ItemID)
	--SystemNotice(role,"??:???????"..BLstate)
	--SystemNotice(role,"??:????????"..BLStateYXJ[BLstate])
	--SystemNotice(role,"??:????????"..BLStateYXJ[statelv])
	if BLstate ~= 0 then
...
function AddBLItem(role,Item,statelv,statetime,NTC) --ntC: NTC?1???????NOTICE,?0???????????????BUFF???????,??0???BUFF,??1 ??BUFF
...
function AddBLandSBItem(role,Item,statelvSB,statelvBL,statetime,NTC)
	...
	if BLstate == statelvBL and SBstate == statelvSB then
		--SystemNotice(role,"??:???????"..statelvSB.."?????1")
		--SystemNotice(role,"??:???????"..statelvBL.."?????1")
		return 1
	end
	if BLstate ~= 0 then
		RemoveState( role , STATE_SBBLGZ )
	...
	--SystemNotice(role,"??:???????"..statelvBL.."???????"..statetime)
	--SystemNotice(role,"??:???????"..statelvSB.."???????"..statetime)
	return ret
end 
...
function TrigASBJY(role,statelv,statetime)
	...
	if SBstate~=0 then
		...
		-- SystemNotice(role,"??:	TrigSb.count"..TrigSb.count)
		TrigSb.id[TrigSb.count]=GetChaDefaultName(role)
			-- SystemNotice(role,"??:	TrigSb.id[TrigSb.count]"..TrigSb.id[TrigSb.count])
		TrigSb.lv[TrigSb.count]= statelv
		-- SystemNotice(role,"??:	statelv"..statelv)
		TrigSb.statetime[TrigSb.count] = statetime
		-- SystemNotice(role,"??:	TrigSb.statetime[TrigSb.count]"..TrigSb.statetime[TrigSb.count])
...
function bulingSB(role)
	...
	for i=1,TrigSb.count,1 do
		-- SystemNotice(role,"??:.roleid"..roleid)
		
		-- SystemNotice(role,"??:.TrigSb.id[TrigSb.count]"..TrigSb.id[TrigSb.count])
		if TrigSb.id[TrigSb.count]== roleid then
			-- SystemNotice(role,"??:.TrigSb.lv[TrigSb.count]"..TrigSb.lv[TrigSb.count])
			-- SystemNotice(role,"??:TrigSb.statetime[TrigSb.count]"..TrigSb.statetime[TrigSb.count])
...
function CanbulingSB(role)
	-- SystemNotice(role,"??:CanbulingSB innnn")
	local SBstate = GetChaStateLv(role,STATE_SBJYGZ)
	if SBstate~=0 then 
...
-- SystemNotice(role,"??:TrigSb.count"..TrigSb.count)
	for i=1,TrigSb.count,1 do
		-- SystemNotice(role,"??:TrigSb.id[TrigSb.count]"..TrigSb.id[TrigSb.count])
		if TrigSb.id[TrigSb.count]== roleid then
		-- SystemNotice(role,"??:CanbulingSB LUA_TRUE")
...
function NewMGH(role,now_tick)
	...
	--SystemNotice(role,"??:pNewMGHID==="..pNewMGHID)
	--SystemNotice(role,"??:MGHID==="..MGHID)
	--SystemNotice(role,"??:MGHure==="..MGHure)
...
function WLface(role,now_tick) 
	local maozi = GetChaItem ( role , 1 , 0 )   --?????????   1???? 0??????
	if maozi == nil  then
...
--[[function WLface(role,now_tick)                     -----------------------???? ,????buff5?1? ??30?1?
	local maozi = GetChaItem ( role , 1 , 0 )   --?????????   1???? 0??????
	if maozi == nil then 
		return
	end
	...
	if maozi_id == 1121 then          --???????
		...
		local a = GetItemAttr( maozi , ITEMATTR_URE )	
		if a > 0 then				
			local bufftime = 15             --5?????,????15??BUFF		
			AddState(role , role ,  STATE_SBBLGZ, 1 , bufftime ) --????
			--SystemNotice( role ,"?BUFF????")
		else 	
			CALCULATE_FUNCTIONS_LUA_000434 = GetResString("CALCULATE_FUNCTIONS_LUA_000434")
			SystemNotice( role , CALCULATE_FUNCTIONS_LUA_000434 )
			RemoveChaItem( role , 1121 , 1 , 0 , -1 , 2 , 1 )	
...
function WLface_KILL(role,now_tick)  --???????
	--SystemNotice( role ,"30???????1???")
	local maozi = GetChaItem ( role , 1 , 0 )   --?????????   1???? 0??????
	if maozi == nil then 
		return
	end
	...
	if maozi_id == 1121  or maozi_id == 1122 then
		...
		a = a-50
		local mm = a/50
		--SystemNotice( role ,"????"..mm)
		SetItemAttr( maozi , ITEMATTR_URE , a )

	end
end]]

function MFDbox(role,now_tick) ------------????????
	local one_minite=60														--??one_minite?60?
	if math.mod(now_tick, one_minite) == 0 and now_tick > 0 then            --??one_minite?now_tick????now_tick??0
		local num_MFDbox=0                                          --
		local item_MFDbox=0	
...
function XZKcheck(role, now_tick)--------------????????
	local item_xzk = GetChaItem(role, 2, 3)
...
function GetTeamLV(role)  				--3 :40-59;4:60-79;5:80??;0 ???????
...
function JJCPaiDui(role,teamNum) -- ???????????????
	local teamlv = GetTeamLV(role)
...
function CanPaiDui(role,Playerments) --????????
	local teamlv = GetTeamLV(role)
	local teamNum = 0 
	local i = 0 
	for i = 1 , 3 , 1 do 
...
function CheckAJJCDate(teamNum,teamlv,Becount) --????????????
...
function RemoveATeam(teamNum,teamlv,Becount)	--?????????,?????
	CALCULATE_FUNCTIONS_LUA_000475 = GetResString("CALCULATE_FUNCTIONS_LUA_000475")
...
function CheckATeamTreeRound1(teamNum,teamlv,pTemp) --??????????????,?????????,????,??????????????????
...
function CheckATeamTree(teamNum,teamlv) --?????????????,??????????,????,??????????????
...
function CanEntryAJJC(teamNum,teamlv)		--???????2?????????
...
function CanInAJJC(teamNum,teamlv)			--??1??????-1 ??2?????? -2 ???????-3 ???????1
...
function NoEntryJJC(teamNum,teamlv,ret)  --????????????????
...
function CheckCanMove(teamNum,teamlv,Becount) --5???????,?????????
	local j = 0
	local valua = 1 
...
function EntryJJC(teamNum,teamlv,map_copy)	--????????????
	local c = 0 
	local d = 0 
	for c = 1 , 2 , 1 do 
...
function NoticePlayer(teamNum,teamlv,map_copy)		--????5????
...
----????
function GivePointGS(teamNum,teamlv,point,Retime)  --?????,?????,?????,????(s)
...
function GivePoint(teamNum,teamlv,Becount)  --?????,?????,??
...
function CheckExPointPer(teamNum,teamlv,Becount)   --??????????????
	...
	for a = 1 , 3 , 1 do  
		...
		for b = 1,	6 ,	1 do 
			...
			for c = 1 , 2 , 1 do 
				...
				JJCteam[a].lv[b].InTeam[c].num = 0  --?????????/??shu
				JJCteam[a].lv[b].InTeam[c].x = 0 			--????x??
				JJCteam[a].lv[b].InTeam[c].y = 0 			--????y??
				JJCteam[a].lv[b].InTeam[c].map = "starena"..a..b --???????
				if a == 3 then 
					if c == 1 then
...
function InitJJCarg()
	...
	for a = 1 , 3 , 1 do 
		...
		JJCarg[a].MaxTime =0	--????
		JJCarg[a].NorTime =0	--????
		JJCarg[a].NorSucPoint = 0 --??????????
		JJCarg[a].NorFalPoint = 0 --lose????????
		JJCarg[a].ExPoint = 0		--?????????
	end
	--????
...
function CanUseItem(role ,Item)
	local ret = 1
	local teamNum,teamlv = IsPlayerInJJCMap(role)   -- IsPlayerInJJCMap(role) :????????????,??????2???????????,???????0,0
...
function CanUseSkill(atk ,skillNo,skillLV)
	local ret = 1
	local posIn = 1
	local teamNum,teamlv = IsPlayerInJJCMap(atk)   -- IsPlayerInJJCMap(role) :????????????,??????2???????????,???????0,0
	if teamNum == 1 or teamNum == 3 then
		posIn= CheckPlayerIsInJJC(atk,teamNum,teamlv ) -- IsPlayerInJJCMap(role) :?????????????????,???????0,????1		
...
function TakePetEndure( role , now_tick)			----6?????
...
function CanAttrReSet(arg)
	...
	if ItemType2 > 27 or IsInITable(ItemType2, equipType)==0 then			--???????
...
function LoveRingTo(role,level)
	...
	elseif level == 5 then	
		LoveBook = GetChaItem2 ( role , 2 , 6959 )
		LoveBook_Energy = GetItemAttr( LoveBook , ITEMATTR_VAL_STR )
		--SystemNotice ( role , "??????????"..LoveBook_Energy )
		...
		--SystemNotice ( role , "??????????"..LoveBook_Energy1 )	
...
function IsGiveXZNewItem(role)
	...
	if nowDays >=90723 and nowDays<=90822 and haveXingZuoka == 0 then    -- 09???????
	...
	while XZNeedItem[NeedId-5881][2*i-1]~= nil do
		...
		if XZNeedItem[NeedId-5881][2*i-1]==4551 or XZNeedItem[NeedId-5881][2*i-1]==1478 then --??????15?1?
...
function GivePointsAndMoney(role,itemid1)
	local a = GetChaItem2(role,2,itemid1) --??????????
	local b = GetItemAttr(a,ITEMATTR_VAL_STR)--GetItemAttr (Item,ITEMATTR_ENERGY)--?????????
	SetItemAttr(a,ITEMATTR_VAL_STR,b+1)--????1
	SynChaKitbag(role,13)
	local num =b+1 --??
...
function Addsafe_exp ( role,decimal,constant,step)	------------------??????????by.????
	...
	elseif decimal ~= nil then 
		local exp_up = GetExp[lv + 1] - GetExp[lv]---??????????
...
function Yurenjie1(role)
    ...
    AddState ( role , role , STATE_CUSI , 10 , 1 ) --??
	RemoveState(role,STATE_CUSI)
...
-- function jsqbijiao(role,now_tick)
	...
	-- local now_month_num= tonumber(now_month)     -------------?	
	-- local now_day_num= tonumber(now_day)	     -------------?
	-- local now_hour_a= tonumber(now_hour)	     -------------?
	-- local now_miniute_b= tonumber(now_miniute)	 -------------?
 	-- local now_second_c=  tonumber(now_second)    -------------?
...
-- end
 ...
 function jsq(role)
    ...
    local a=now_month_num*100+now_day_num
		...
		--SystemNotice(role,"????????"..now_month_num.."?"..now_day_num.."?"..now_hour_a.."?"..now_miniute_b.."?")
	local chaitem = GetChaItem2(role,2,8736)
		SetItemAttr(chaitem,ITEMATTR_VAL_PARAM1,a)---????	
        SetItemAttr(chaitem,ITEMATTR_VAL_PARAM2,zbiaomap)--????
	return LUA_TRUE
end
 ...
 function wcjsq(role)--qq
	...
	elseif zbiaomapa-fzsj <= 3 then   --------------------------????3??
		GiveItem ( role , 0 , 5639, 3 , 4 )   -------------------?????????*3
		SystemNotice(role,"Time for finishing the quest is"..zbiaomapa-fzsj.."minutes.")
		DelBagItem ( role, 8736, 1)
		
	elseif  zbiaomapa-fzsj == 4  then -------------------------=4??
		GiveItem ( role , 0 , 849, 3, 4 )    -------------------???????*3
		SystemNotice(role,"Time for finishing the quest is"..zbiaomapa-fzsj.."minutes.")
		DelBagItem ( role, 8736, 1)
		
	elseif  zbiaomapa-fzsj == 5  then -------------------------=5??
		GiveItem ( role , 0 , 5644, 3, 4 )   -------------------????????*3
		SystemNotice(role,"Time for finishing the quest is"..zbiaomapa-fzsj.."minutes.?")
		
		DelBagItem ( role, 8736, 1)
	elseif  zbiaomapa-fzsj == 6  then -------------------------=6??
		GiveItem ( role , 0 , 2890, 3, 4 )   -------------------?????????*3
		SystemNotice(role,"Time for finishing the quest is"..zbiaomapa-fzsj.."minutes.?")
		DelBagItem ( role, 8736, 1)
		
	elseif  zbiaomapa-fzsj >= 7  then -------------------------==7??
		GiveItem ( role , 0 , 2891, 3, 4 )   -------------------???????*3
...
function CheskStats(role)
	...
	local QuerPK = "SELECT pk, KillMobs, reset FROM GameDB.dbo.character WHERE cha_name = '"..NameCha.."'"
	if Connect == 1 then	
		local Success, Query = LuaSQL("query", ConnectID, QuerPK)
		if Success == 1 then
				local Data = LuaSQL("fetch", ConnectID, Query)
				HelpInfo(role, 0, "***********\194\224\248\224 \241\242\224\242\232\241\242\232\234\224***********_Z PK = "..Data["pk"].."._Óáèòî ìîáîâ - "..Data["KillMobs"].."_Ðåñåò - "..Data["reset"])
		else
			--SystemNotice(role, "Error Stats. Ñîîáùèòå àäìèíèñòðàöèè.")
			PopupNotice(role, "Error Stats. \209\238\238\225\249\232\242\229 \224\228\236\232\237\232\241\242\240\224\246\232\232.")
		end
	else
