	...
	for i = 1, PlayerNum, 1 do
		...
		if (ValidCha(Player) == 1) then
			...
			if GetChaAttr(Player, ATTR_HP) <= 0 then
				BickerNotice(Player, "You're being revived in "..PTB.MapCopy[CopyID].Players[PID].deadtimer.." second(s).")
...
PTB.StartRound 										= function(MapCopy)
	...
	if PTB.MapCopy[CopyID].Active == true then
		...
		MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: Round #"..PTB.MapCopy[CopyID].Round.." has started!")
	end
end
	...
	if (side_id == 2) then
		...
		if (in_zone_1 == true) then
			if (plant_time_1 > 0) then
				...
				MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: WARNING!!! The bomb will be planted by the Red Team at \"Zone 1\" in "..PTB.MapCopy[CopyID].PlantTime1.." second(s)!")
			else
		...
		if (in_zone_2 == true) then
			if (plant_time_2 > 0) then
				...
				MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: WARNING!!! The bomb will be planted by the Red Team at \"Zone 2\" in "..PTB.MapCopy[CopyID].PlantTime2.." second(s)!")
			else
...
PlantTheBombInZone_1 								= function(Player)
	...
	if (side_id == 2) then
		...
		if (in_zone_1 == true) then
			...
			MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: Red Team planted the bomb at \"Zone 1\". The explosion will occur in "..ToClockFormat(PTB.MapCopy[CopyID].Time.Round).."!")
		end
	end
end
PlantTheBombInZone_2 								= function(Player)
	...
	if (side_id == 2) then
		...
		if (in_zone_2 == true) then
			...
			PTB.MapCopy[CopyID].BombStatus			= 1		
			PTB.MapCopy[CopyID].Time.Round 			= PTB.Conf.Time.Defuse
			MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: Red Team planted the bomb at \"Zone 2\". The explosion will occur in "..ToClockFormat(PTB.MapCopy[CopyID].Time.Round).."!")
		end
	end
end
	...
	if (side_id == 1) then
		...
		if (in_zone_1 == true) then
			if (defuse_time_1 > 0) then
				...
				MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: Green Team trying to defuse the bomb at \"Zone 1\". Remaining time: "..PTB.MapCopy[CopyID].DefuseTime1.." second(s)!")
			else
				...
				MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: The bomb has been defused by the Green Team granting [+1] point for them.")
			end
		else
			PTB.MapCopy[CopyID].DefuseTime1			= PTB.Conf.Time.Defusing
		end
	...
	if (side_id == 1) then
		...
		if (in_zone_2 == true) then
			if (defuse_time_2 > 0) then
				...
				MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: Green Team trying to defuse the bomb at \"Zone 2\". Remaining time: "..PTB.MapCopy[CopyID].DefuseTime2.." second(s)!")
			else
				KillCha(PTB.MapCopy[CopyID].Bomb)
				PTB.AddPoint(MapCopy, "Green", 1)
...
PTB.Finish 											= function(MapCopy)
	MapCopyNotice(MapCopy, "["..PTB.Conf.MapName.."]: This match has come to an end, hope you enjoyed!")
	local CopyID 									= GetMapCopyID2(MapCopy)
	local PlayerNum									= GetMapCopyPlayerNum(MapCopy)
	BeginGetMapCopyPlayerCha(MapCopy)
	for i = 1, PlayerNum, 1 do
...
PTB.Register 										= function(Player)
	...
	if Func.CheckList(Player) == 1 then
		BickerNotice(Player, "You're already in the waiting list, be patient.")
		return
	end
	if PTB.OpenDay() == 0 then
		BickerNotice(Player, "Registering for "..PTB.Conf.MapName.." is unavailable today, please come back again tommorow.")
		return
	end
	if PTB.OpenHour() == 0 then
		BickerNotice(Player, "Registering for "..PTB.Conf.MapName.." is unavailable at this hour, please come back later.")
		return
	end		
	...
	if PTB.Conf.MinLv > GetChaAttr(Player, ATTR_LV) or (PTB.Conf.MaxLv ~= 0 and PTB.Conf.MaxLv < GetChaAttr(Player, ATTR_LV)) then
		BickerNotice(Player, "You can't participate, only players between levels "..PTB.Conf.MinLv.." through "..PTB.Conf.MaxLv.." can participate.")
		return
	end	
	if PTB.Log(Player, 2) == 0 then
		BickerNotice(Player, "You can't participate any more for this day.")
		return
	end	
	Func.Push(Player)
	BickerNotice(Player, "You have registered for "..PTB.Conf.MapName.." successfully, please wait for it to start.")
end