...
function GetChaName1_born2 ( role,npc )
	local cha_type = GetChaTypeID ( role )
        if cha_type~=1 and cha_type~=3 and cha_type~=2 then
...
function GiveItem_zsbook2 ( role )
	...
	---SystemNotice( role ,""..cha_job )
	if cha_job == 8 then
		GiveItem( role , 0 , 16013 , 1 , 4 )--книга второго перерождения чемпиона
	elseif cha_job == 9 then
		GiveItem( role , 0 , 16012 , 1 , 4 )--книга второго перерождения воителя
	elseif cha_job == 12 then
		GiveItem( role , 0 , 16017 , 1 , 4 )--книга второго перерождения стрелка
	elseif cha_job == 13 then
		GiveItem( role , 0 , 16015 , 1 , 4 )--книга второго перерождения цилительници
	elseif cha_job == 14 then
		GiveItem( role , 0 , 16014 , 1 , 4 )--книга второго перерождения колдуньи
	elseif cha_job == 16 then
		GiveItem( role , 0 , 16016 , 1 , 4 )--книга второго перерождения покорителя морей
	end
end
...
function MegaBuff( role )
	...
	if (StateType1 == 0 and  StateType2 == 0 and  StateType3 == 0 and  StateType4 == 0 and  StateType5 == 0 and  StateType6 == 0) then
		...
		PopupNotice( role ,"Вы получили заряд положительной энергии на 1 час! ") 
	else

		PopupNotice(role,"Баффы уже наложены! ") 
